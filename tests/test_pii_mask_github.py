...
class TestHashHelpers:
    ...
    def test_hash_email_preserves_structure(self):
        result = _hash_email("john@org_name.com")
        assert "@" in result
...
class TestMaskBody:
    ...
    def test_masks_emails_in_text(self):
        result = _mask_body("contact john@org_name.com for details")
        assert "john@org_name.com" not in result
        assert "@example-health.com" in result
...
class TestMaskPR:
    def _sample_pr(self):
        return {
            ...
            "author": "amitchhajer",
            "author_id": 427888,
            "body": "See john@org_name.com for context",
            "assignees": ["alice", "bob"],
            ...
            "commits": [
                {"sha": "abc123", "message": "fix for @johndoe",
                 ...
                 "author_email": "amit@org_name.com",
    ...
    def test_hashes_title_and_body(self):
        ...
        assert pr["body"] != "See john@org_name.com for context"
    ...
    def test_hashes_commit_message(self):
        ...
        assert "amit@org_name.com" not in commit["author_email"]
...
class TestMaskRepoMetadata:
    def test_masks_org_in_full_name(self):
        meta = mask_repo_metadata({
            "full_name": "org_name/MyRepo",
            "description": "Contact admin for help",
        })
        assert "org_name" not in meta["full_name"]
...
class TestPipelineEndToEnd:
    def test_masks_all_file_types(self, s3_env):
        ...
        src.upload_json({
            ...
            "comments": [], "commits": [
                {"sha": "aaa", "message": "init",
                 "author_name": "Alice", "author_email": "alice@org_name.com",
        ...
        src.upload_json({
            "full_name": "org_name/repo", "description": "test repo",
        }, "github/org_name__repo/repo_metadata.json")

        src.upload_json({"total": 1}, "github/org_name__repo/_stats.json")
        ...
        # Verify PR
        pr = dst.download_json("github/org_name__repo/prs/1.json")
        assert "alice" not in pr["author"]
        assert "alice@org_name.com" not in pr["commits"][0]["author_email"]

        # Verify contributors
        contribs = dst.download_json("github/org_name__repo/contributors.json")
        assert "alice" not in contribs[0]["login"]
        assert contribs[0]["id"] == 0

        # Verify metadata
        meta = dst.download_json("github/org_name__repo/repo_metadata.json")
        assert "org_name" not in meta["full_name"]

        # Verify stats copied unchanged
        stats = dst.download_json("github/org_name__repo/_stats.json")
    ...
    def test_checkpoint_resume(self, s3_env):
        ...
        cp.mark_item_done("mask", "github/org_name__r/prs/1.json")
        cp.save(force=True)
        # Write PR 1 to dst as-is (simulating prior run)
        dst.upload_json({"number": 1, "author": "a"}, "github/org_name__r/prs/1.json")
        ...
        pr1 = dst.download_json("github/org_name__r/prs/1.json")
        assert pr1["author"] == "a"

        # PR 2 should be masked
        pr2 = dst.download_json("github/org_name__r/prs/2.json")

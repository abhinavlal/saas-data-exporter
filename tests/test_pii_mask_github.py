...
class TestOrgReplacement:
    ...
    def test_rewrite_key(self):
        assert _rewrite_key("github/org_name__repo/prs/1.json") == \
            "github/medica__repo/prs/1.json"
        assert _rewrite_key("github/org_name__MyService/contributors.json") == \
            "github/medica__MyService/contributors.json"

    def test_preserves_non_org_name_keys(self):
...
class TestPipelineEndToEnd:
    def test_masks_all_file_types_and_rewrites_keys(self, s3_env):
        ...
        # Upload sample data under org_name__ prefix
        src.upload_json({
        ...
        # Keys should be rewritten: org_name__ → medica__
        ...
        # html_url should have org_name replaced with medica
        assert "org_name" not in pr["html_url"]
        ...
        assert dst.download_json("github/org_name__repo/prs/1.json") is None

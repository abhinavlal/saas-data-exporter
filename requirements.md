...
Four standalone Python scripts that export data from Practo's SaaS tools via their APIs. Each script produces structured JSON + CSV output that feeds into the PII scrubbing pipeline (`practo_pii_scrub.py`).
...
└── slack/
    ...
    └── slack-practo-members.csv  # Workspace member list
```
...
```bash
uv run main.py --user someone@practo.com --key service-account.json --output ./export
uv run main.py --user someone@practo.com --key service-account.json --emails 1000 --events 200 --files 100
uv run main.py --user someone@practo.com --key service-account.json --skip-gmail --skip-calendar
```
...
| Argument | Default | Description |
|----------|---------|-------------|
| `--user` | required | Target user email (e.g. `someone@practo.com`) |
...
```python
JIRA_DOMAIN = "practo.atlassian.net"
JIRA_EMAIL = "abhinav@practo.com"
PROJECTS = ["IES"]
...
### Companion: `slack-practo-members.csv`
...
After export, files are copied into `practo-data/` for scrubbing:

```
source-data-exporter/github/github-export/repo1/   →  practo-data/github/repo1/
source-data-exporter/google-workspace/export/user*/ →  practo-data/google/user*/
source-data-exporter/jira/jira-export/IES/          →  practo-data/jira/IES/
source-data-exporter/slack/slack-exports/            →  practo-data/slack/slack-exports/
source-data-exporter/slack/slack-practo-members.csv  →  practo-data/slack/slack-practo-members.csv
salesforce/ (Data Export tool)                        →  practo-data/salesforce/sf_samples/
```

The git repository clone is placed at `practo-data/github/repo1/code/` (not exported by the GitHub script — cloned separately with `git clone`).
